...
export function setup() {
  ...
  const runtime = new SurvivorRuntime({ bus, entityMgr, buffSystem, arena, displacer, combatResolver, player }, () => 0.4)
...
describe('survivor combat', () => {
  ...
  it('death explosions kill nearby enemies once without recursive chain explosions', () => {
    ...
    expect(runtime.gems).toHaveLength(2)
  })
...
it('runs a complete evolved build through the eight-minute monster ramp', () => {
  ...
  for (let i = 0; i < 30001 && !runtime.result; i++) {
    ...
    expect(runtime.gems.length).toBeLessThanOrEqual(450)
  }
